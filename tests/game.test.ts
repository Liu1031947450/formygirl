...
import { advanceJourney, cameraAt, createJourney, DETOURS, escapePosition, letterPoints, movementVector, movePlayer, OBSTACLE_MESSAGES, pathLength, pathPosition, pathSection, roadAt, ROOMS, ROUTE_SAMPLES, VIEW, WALK_RADIUS, WALK_SPEED, WORLD } from '../src/game.ts';
import type { Direction, Journey, Point } from '../src/game.ts';

function walkTo(roomIndex: number, journey: Journey, target: Point, recorded?: Set<string>) {
  for (let frame = 0; frame < 2000; frame += 1) {
    const horizontal = target.x - journey.position.x;
    const vertical = target.y - journey.position.y;
    if (Math.hypot(horizontal, vertical) < 0.1 || journey.completed) return;
    ...
    const elapsed = Math.min(1 / 60, Math.min(...remaining) * Math.sqrt(directions.length) / WALK_SPEED);
    if (recorded) for (const point of movePlayer(roomIndex, journey.position, directions, elapsed)) recorded.add(`${point.x},${point.y}`);
    assert.ok(advanceJourney(roomIndex, journey, directions, elapsed), `stuck in ${roomIndex}: ${JSON.stringify(journey.position)} -> ${JSON.stringify(target)}`);
...
function walkPath(roomIndex: number, journey: Journey, points: readonly Point[], recorded?: Set<string>) {
...
test('all four rooms and all twenty dead ends work with directional input and retain only real trace points', () => {
  ...
  for (const [roomIndex, room] of ROOMS.entries()) {
    ...
    for (const branch of DETOURS[roomIndex]) {
      walkPath(roomIndex, journey, pathSection(room.points, distance, branch.at), recorded);
      const before = journey.percent;
      const hadKey = journey.hasKey;
      walkPath(roomIndex, journey, branch.points, recorded);
      ...
      for (let attempt = 0; attempt < 20; attempt += 1) advanceJourney(roomIndex, journey, directions, 1);
      assert.ok(Math.hypot(journey.position.x - position.x, journey.position.y - position.y) < 0.1, 'cannot pass a blocked end');
      walkPath(roomIndex, journey, [...branch.points].reverse(), recorded);
      assert.equal(journey.percent, before);
      assert.equal(journey.hasKey, hadKey);
      distance = branch.at;
    }
    walkPath(roomIndex, journey, pathSection(room.points, distance, pathLength(room.points)), recorded);
